...
L2 = 0  # 1e-5
NUM_EPOCH = 100
writer = SummaryWriter('logs/tensorboardX_log/3')
...
def train():
    ...
    for epoch in range(num_epoch):
        ...
        for i, (text, image, label) in tqdm(enumerate(train_loader)):
            ...
            # dot = make_dot((text_aligned, image_aligned, _), params=dict(similarity_module.named_parameters()))
            # dot.render('./images/similarity')
            # exit(0)
        ...
        # --- Visualize --- # 
        writer.add_scalar('loss_similarity_train', loss_similarity_train, epoch + 1)
        writer.add_scalar('loss_detection_train', loss_detection_train, epoch + 1)
        writer.add_scalar('acc_similarity_train', acc_similarity_train, epoch + 1)
        writer.add_scalar('acc_detection_train', acc_detection_train, epoch + 1)
        ...
        # --- Visualize --- # 
        writer.add_scalar('loss_similarity_test', loss_similarity_test, epoch + 1)
        writer.add_scalar('loss_detection_test', loss_detection_test, epoch + 1)
        writer.add_scalar('acc_similarity_test', acc_similarity_test, epoch + 1)
        writer.add_scalar('acc_detection_test', acc_detection_test, epoch + 1)
    ...
    # --- close writer of visulize ---# 
    writer.close()


    # --- save model for production --- #
    # test(similarity_module, detection_module, test_loader)
    torch.save(similarity_module.state_dict(), 'similarity_model.pth')
    torch.save(detection_module.state_dict(), 'detection_model.pth')
